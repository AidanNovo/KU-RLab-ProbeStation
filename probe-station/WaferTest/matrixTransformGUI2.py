...
class Window(Frame):
    ...
    def __init__(self, master=None, motorsPort='COM4', keithleyPort='COM6', emulate=False):
        ...
        raiseButton.place(x=100, y=325)
        lowerButton.place(x=100, y=375)
        ...
        raiseFastButton.place(x=10, y=325)
        lowerFastButton.place(x=10, y=375)
...
##        self.NeedleTest.place(x=700,y=110)
    ...
    def moveToPosition(self,x=-10000,y=-10000, zRetraction=3):
    ...
    def setHome(self):
        ...
        self.motors.setSafetyLimit(motor='z',min=None,max=1.5)
...
##                self.motors.moveFor('z',z)
    ...
    def readResistance(self, debug=True, points=10):
        ...
        results = self.kt.measresist(debug=debug,points=points)
        resist = results['R']
    ...
    def saveResistance(self,sensor="defaultSensor", padNumber=-1, notes="", x=-1, y=-1, results=None):
        if results is None:
    ...
    def saveResistanceManual(self):
        ...
        self.saveResistance(sensor,padNumber,notes,x,y,self.readResistance(debug=True))
    ...
    def writeResistance(self):
        try:
            ...
            to_unicode = str
        sensor = str(self.sensorEntry.get())
        filename = sensor+"_resistance.json"
        ...
        with open(filename,'a',encoding='utf8') as f:
           ...
           f.write(to_unicode(dump))
        print("Writing Resitance for sensor: "+sensor+" to file: "+filename)
    ...
    def measureAllPoints(self,test=False):
        ...
        if test:
            ...
            if not os.path.exists("img/"+sensor):
                os.makedirs("img/"+sensor)
        ...
        for p in self.resistanceMeasurement['Pads']:
            ...
            #Refresh lastContactZ when far from last set
            if p.get('Pad') < 2:
                self.motors.moveTo('z',0.0)
                if self.raiseToSoftContact(step=0.05,maxMovement=1.75):
                    lastContactX = self.motors.getPosition('x')
                    lastContactY = self.motors.getPosition('y')
                    lastContactZ = self.motors.getPosition('z')
                    print(f"Setting {lastContactZ:.3f} as lastContactZ")
                    ##save the results from the first contact
                    if self.raiseToContact(step=0.05,maxMovement=1.0):
                        results = self.getMinResistance()
                        ##results = self.getMinResistance2()
                        resistances.append(results['R'])
                        self.saveResistance(sensor,p.get('Pad'),"",x_given,y_given,results)
                        attempt += 1

            while attempt <= 0: ## default is 6 for BNL
                self.motors.moveTo('z',lastContactZ-0.15)
                if attempt>0:
                    x_rand = x_given + random.uniform(-0.02,0.02)
                    y_rand = y_given + random.uniform(-0.02,0.02)
                    self.moveToPosition(x=x_rand,y=y_rand)
                
                attempt += 1
                if self.raiseToSoftContact(step = 0.05, maxMovement=1.25):
                    lastContactX = self.motors.getPosition('x')
                    lastContactY = self.motors.getPosition('y')
                    lastContactZ = self.motors.getPosition('z')
                    print(f"Setting {lastContactZ:.3f} as lastContactZ")
                if self.raiseToContact(step=0.05, maxMovement=1.0):
                    ...
                    while frame is None:
                        ...
                        cv2.imwrite(f"img/"+sensor+"/Pad"+str(pad)+"_"+str(attempt)+".png", frame)
                    results = self.getMinResistance()
                    resistances.append(results['R'])
                    self.saveResistance(sensor,p.get('Pad'),"",x_given,y_given,results) # save it in any case, we take care in data analysis
                else:
                    print("Possible Out of Alignment for point: ("+str(x_given)+","+str(y_given)+")")
                    continue    #if raiseToContact fails, skip this point and try again at the next

                if len(resistances) >= 3:
                    std = np.std(resistances)
                    mean = np.mean(resistances)
                    if std < 0.2*mean:
                        ##break out of this pad
                        self.motors.moveTo('z',-0.1)
                        break
                    else:
                        ##remove resistance that is farthest from the mean
                        resistances.remove(max(resistances,key=lambda x:abs(x-mean)))
                self.motors.moveTo('z',-0.25)
...
##                return result
    ...
    def NeedleTest(self):
        ...
        for attempt in range(0, attempts):
            ...
            self.saveResistance(padNumber=attempt, notes="Needle Stress Test", x=0, y=0,results=self.readResistance(debug=True))
            frame = None
            while frame is None:
                time.sleep(0.2)
                ret, frame = cam.read()
